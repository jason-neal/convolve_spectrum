...
@pytest.mark.parametrize("wav, wav_min, wav_max", [
    ([np.nan], 1, 2),
    ([np.inf], 1, 2),
    ([-1*np.inf], 3, 4)])
...
def test_fast_convolution():
    ...
    for a_val in a:
        ...
        assert fast_convolve(a_val, resolution, b, c, 5) == 1  # Test a flat input of 1s gives a flat output of 1s
        assert fast_convolve(a_val, resolution, b, 0 * c, 5) == 0  # Test a flat input of 1s gives a flat output of 1s
...
def test_IPcovnolution_depreciation():
    a = np.linspace(2130, 2170, 1024)
    b = np.linspace(2100, 2200, 1024)
